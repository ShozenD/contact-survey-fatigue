...
#' @export
varselect_s1_stepwise <- function(data, config) {
  cat( "Running initial screening model...\n")
  post_init <- varselect_s1_initial(df, config)

  cat( "Running stepwise variable selection...\n")
  stan_data <- make_stan_data_varselect(data, config)
  config_stepwise <- config$s1$stepwise_model
  ...
  for (i in 1:nrow(post_init)) {
    ...
    stan_data_update <- update_stan_data(i, 1, stan_data, post_init)
  ...
  selected_vars <- colnames(update_stan_data(which.max(loo_elpd), 1, stan_data, post_init)$X)
  post_best <- data.table::setDT(posterior::summarise_draws(fit_best$draws(c("beta0", "beta"))))
  post_best[, varname := c("Intercept", selected_vars)]
