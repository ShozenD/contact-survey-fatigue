...
clean_labels <- function(x) {
  ...
  variable <- factor(variable, levels = c("Age group", "Gender", "Household size", "Education", "Employment status", "Symptoms"))
  ...
  labels <- case_when(
    x == "preschool_0-5" ~ "Preschool [0-5]",
    x == "raised_at_home_0-5" ~ "Raised-at-home-toddler [0-5]",
    x == "student_6-9" ~ "Student [6-9]",
    x == "student_10-14" ~ "Student [10-14]",
