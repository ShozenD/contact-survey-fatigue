...
# Preprocessing
# ====================
# Participants in wave 21

# Participants in wave 21 who participated before wave 21
ids_part_w21 <- df_part %>% filter(wave == 21) %>% pull(new_id) %>% unique()
ids_part_w1t21 <- df_part %>% filter(wave < 21) %>% pull(new_id) %>% unique()
ids_part_rep <- intersect(ids_part_w21, ids_part_w1t21)
...
# Save data
# ====================
saveRDS(df_cnt, file.path("data", "silver", "covimod_wave_21.rds"))
