...
# Preprocessing
# ====================
# Participants in wave == WAVE
df_part_wave <- df_part %>% filter(wave == WAVE)

# Participants in the wave who participated before that wave
ids_unique_wave <- df_part_wave %>% pull(new_id) %>% unique()                            # Unique participants in wave
ids_unique_from_wave1 <- df_part %>% filter(wave < WAVE) %>% pull(new_id) %>% unique()   # Unique participants in waves 1 to WAVE - 1
ids_repeat <- intersect(ids_unique_wave, ids_unique_from_wave1)                          # Participants who participated in waves 1 to WAVE - 1 and wave WAVE

# Add repeat status
df_part_wave <- df_part_wave %>%
  mutate(repeat_status = ifelse(new_id %in% ids_repeat, 1, 0))
print(mean(df_part_wave$repeat_status))
length(df_part_wave$repeat_status) - sum(df_part_wave$repeat_status)
...
# Save data
# ====================
saveRDS(df_cnt, file.path("data", "silver", "covimod_varselect_wave_21.rds"))
