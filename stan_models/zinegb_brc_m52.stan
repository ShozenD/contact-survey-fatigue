...
model
{
  ...
  for (i in 1:Nzero) {
    int idx = zero_idx[i];
    target += log_mix(theta[idx], 0, neg_binomial_2_lpmf(Y[idx] | exp(log_lambda_obs[idx]), 1/reciprocal_phi));
  }

  // Non-zero case
  target += log1m(theta[non_zero_idx]);
  target += neg_binomial_2_lpmf(Y[non_zero_idx] | exp(log_lambda_obs[non_zero_idx]), 1/reciprocal_phi);
}
...
generated quantities {
  ...
  for (i in 1:Nzero) {
    int idx = zero_idx[i];
    log_lik[idx] = log_mix(theta[idx], 0, neg_binomial_2_lpmf(Y[idx] | exp(log_lambda_obs[idx]), 1/reciprocal_phi));

    // Generate predictions
    { // Local scope
      real Z = bernoulli_rng(1 - theta[idx]);
      if (Z > 0) {
        yhat[idx] = neg_binomial_2_rng(exp(log_lambda_obs[idx]), 1/reciprocal_phi);
      } else {
        yhat[idx] = 0;
      }
    }
  ...
  for (i in 1:Nnonzero) {
    ...
    log_lik[idx] = log1m(theta[idx]) + neg_binomial_2_lpmf(Y[idx] | exp(log_lambda_obs[idx]), 1/reciprocal_phi);
