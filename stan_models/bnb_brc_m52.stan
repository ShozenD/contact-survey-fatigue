...
parameters
{
  ...
  vector<lower=0>[Npart] reciprocal_phi; // Reciprocal of the dispersion parameter
...
model
{
  ...
  target += beta_lpdf(reciprocal_phi | 0., 1);
  ...
  // ========== Likelihood ==========
  target += neg_binomial_2_lpmf(Y | exp(log_lambda_obs + 1e-4), 1.0/reciprocal_phi[part_idx]);
}
...
generated quantities {
  ...
  // Non-zero case
  for (i in 1:Nobs) {
    log_lik[i] = neg_binomial_2_lpmf(Y[i] | exp(log_lambda_obs[i] + 1e-4), 1.0/reciprocal_phi[part_idx[i]]);
    yhat[i] = neg_binomial_2_rng(exp(log_lambda_obs[i] + 1e-4), 1.0/reciprocal_phi_obs[part_idx[i]]);
  }
}
