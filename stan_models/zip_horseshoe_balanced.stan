...
parameters {
  ...
  /* ===== Repeat effects ===== */
  // Horseshoe prior
  vector<lower=0>[P] zb_rp;        // Horseshoe auxiliary random variables
  vector<lower=0>[P] hs_local_rp;  // Horseshoe local parameters
  real<lower=0> hs_global_rp;      // Global shrinkage parameters
  real<lower=0> hs_slab_rp;        // Slab regularization parameter
}

transformed parameters {
  /* ===== Fixed effects ===== */
  vector[P] beta0;  // population-level effects
  ...
  beta1 = horseshoe(zb_rp, hs_local_rp, hs_global_rp, hs_scale_slab^2 * hs_slab_rp);
}
...
model {
  ...
  // ===== Repeat effects =====
  target += std_normal_lpdf(zb_rp);
  target += student_t_lpdf(hs_local_rp | hs_df, 0, 1) - rows(hs_local_rp) * log(0.5);
  target += student_t_lpdf(hs_global_rp | hs_df_global, 0, hs_scale_global) - 1 * log(0.5);
  target += inv_gamma_lpdf(hs_slab_rp | 0.5 * hs_df_slab, 0.5 * hs_df_slab);
  ...
  // ===== Likelihood =====
  {
    ...
    // Non-zero case
    target += log1m(theta01) + poisson_lpmf(y01 | exp(log_lambda01));
    target += log1m(theta11) + poisson_lpmf(y11 | exp(log_lambda11));
  }
}
