...
```{r}
dt_part_w21 <- dt_part %>% filter(wave == 21)

# Participants in wave 4 who participated before wave 4
ids_part_w21 <- dt_part_w21 %>% pull(new_id) %>% unique()
ids_part_w1t21 <- dt_part %>% filter(wave < 21) %>% pull(new_id) %>% unique()
ids_part_rep <- intersect(ids_part_w21, ids_part_w1t21)

dt_part_w21 <- dt_part_w21 %>% mutate(rep = ifelse(new_id %in% ids_part_rep, TRUE, FALSE))
...
ggsave("../figures/wave21-hhsize-dist.pdf", units = "cm", width = 14.26, height = 12)
```
