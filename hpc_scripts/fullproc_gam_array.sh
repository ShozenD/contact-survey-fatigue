...
# Create main script
cat > "$OUT_PATH/fullproc_gam_array.pbs" <<EOF
#!/bin/bash
...
# Create datasets
Rscript scripts/preproc_gam_data_array.R --config "$CONFIG_FILE" --arr_idx \$PBS_ARRAY_INDEX

# Run Stan model
Rscript scripts/run_stan_gam_array.R --config "$CONFIG_FILE" --arr_idx \$PBS_ARRAY_INDEX
