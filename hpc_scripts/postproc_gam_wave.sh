...
CONFIG_FILE="negb_gam_wave_cap15.yaml"

# Create main script
cat > "$OUT_PATH/postproc_gam_wave.pbs" <<EOF
#!/bin/bash
...
# Postprocess
Rscript scripts/postproc_gam_wave.R --config "$CONFIG_FILE" --arr_idx \$PBS_ARRAY_INDEX
EOF

# Execute main script
cd $OUT_PATH
qsub "postproc_gam_wave.pbs"